...
| input_atom | true / false | false | 入力をアトムとする(true)か文字コードリストとする(false)かを指定する。<br />本オプションは入力が`[]`のみの場合の挙動にのみ影響を与える。<br/>trueを指定した場合、パースエラーにならず空リストとなる。<br/>falseを指定した場合は空文字列とみなされパースエラーとなる。 |
